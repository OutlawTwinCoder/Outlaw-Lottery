...
public class LotteryCommand implements CommandExecutor, TabCompleter {
    ...
    private void sendUsage(CommandSender sender) {
        sender.sendMessage(Messages.get(plugin.getLanguage(), "prefix")
                + "/outlawlottery language <fr|en>, create npc, remove npc, setprice <amount>, settime now, makelottery now, debug");
    }
    ...
    private void handleRemove(CommandSender sender, String[] args) {
        ...
        if (args.length >= 2 && args[1].equalsIgnoreCase("npc")) {
            NpcManager manager = plugin.getNpcManager();
            if (manager.getNpcId() == null) {
                sender.sendMessage(Messages.get(plugin.getLanguage(), "prefix") + Messages.get(plugin.getLanguage(), "no-npc"));
                return;
            }
            manager.deleteNpc();
            sender.sendMessage(Messages.get(plugin.getLanguage(), "prefix") + Messages.get(plugin.getLanguage(), "npc-removed"));
        } else {
            sender.sendMessage(Messages.get(plugin.getLanguage(), "prefix") + "/outlawlottery remove npc");
        }
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            return Arrays.asList("language", "create", "remove", "setprice", "settime", "makelottery", "debug");
        }
        if (args.length == 2) {
            return switch (args[0].toLowerCase(Locale.ROOT)) {
                case "language" -> Arrays.asList("en", "fr");
                case "create" -> Arrays.asList("npc");
                case "remove" -> Arrays.asList("npc");
